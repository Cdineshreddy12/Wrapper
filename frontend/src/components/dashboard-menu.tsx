...
function ApplicationsTab() {
    ...
    /**
     * Handle viewing application details - navigate to page
     */
    const handleViewApp = (app: any) => {
        navigate(`/dashboard/applications/${app.appId}`);
    };
                                ...
                                <CardContent className="p-4 flex flex-col items-center justify-center min-h-[200px]">
                                    <div className="flex flex-col items-center w-full space-y-3">
                                        ...
                                        <div className="text-center w-full">
                                            <Typography variant="h4" className="text-sm font-semibold mb-1">{app.appName || 'Unknown App'}</Typography>
                                            <Badge className={getStatusColor(app.isEnabled ? 'active' : 'inactive')} variant="outline">
                                                {app.isEnabled ? 'Active' : 'Inactive'}
                                            </Badge>
                                        </div>
                                    </div>

                                    <IconButton variant="ghost" size="sm" className="w-full mt-auto" startIcon={Eye}>
                                        View Details
                                    </IconButton>
                                </CardContent>
                            </Card>
                                        ...
                                        <div>
                                            ...
                                            <div className="space-y-4">
                                                {selectedApp.modules.map((module: any) => {
                                                    // Normalize enabledModules to array (defensive)
                                                    const normalizedEnabledModules = Array.isArray(selectedApp.enabledModules) 
                                                      ? selectedApp.enabledModules 
                                                      : [];
                                                    const isModuleEnabled = normalizedEnabledModules.includes(module.moduleCode);
                                                                ...
                                                                <div className="space-y-2">
                                                                    ...
                                                                    <div className="flex flex-wrap gap-1">
                                                                        {module.permissions.map((permission: any, index: number) => {
                                                                            ...
                                                                            // Normalize modulePermissions to string array (extract codes from objects if needed)
                                                                            const permissionCodes = Array.isArray(modulePermissions) 
                                                                              ...
                                                                              : [];
                                                                            const isEnabled = isModuleEnabled && (permissionCodes.includes(permissionText) || (Array.isArray(customPermissions) && customPermissions.includes(permissionText)));
                                                                            return (
                                                                                <ThemeBadge
                                                                                    key={index}

...
/**
 * 🔍 **DEBUG PERMISSIONS STRUCTURE SCRIPT**
 * 
 ...
 * 
 * Run with: node debug-permissions-structure.js
 */
...
async function debugPermissionsStructure() {
  try {
    ...
    roles.forEach(role => {
      if (role.permissions && Array.isArray(role.permissions)) {
        role.permissions.forEach(permission => {
          ...
          // Try to extract app code from permission
    ...
    users.forEach(user => {
      const userRoles = userRoles.filter(ur => ur.userId === user.userId && ur.isActive);

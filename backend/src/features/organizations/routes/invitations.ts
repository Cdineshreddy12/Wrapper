...
export default async function invitationRoutes(
  ...
  fastify.get('/debug/url-generation', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
  ...
  fastify.post('/create-test-invitation', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const body = request.body as Record<string, unknown>;
      const { orgCode, email, roleName = 'Member' } = body;
      ...
      const [tenant] = await db
        ...
        .from(tenants)
        .where(eq(tenants.kindeOrgId, orgCode as string))
        .limit(1);
  ...
  fastify.post('/test-email', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      return reply.code(500).send({
        ...
        message: error.message,
        details: (error as Error & { response?: { data?: unknown } }).response?.data
      });
    }
  });

  // Test endpoint to verify invitation system with working user assignment
  fastify.post('/test-invitation-flow', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const body = request.body as Record<string, unknown>;
      const { orgCode, kindeUserId, email } = body;
      ...
      if (orgResult.success) {
        ...
        try {
          ...
          const acceptanceError = err as Error;
          acceptanceResult = {
            success: false,
  ...
  fastify.get('/debug/:org/:email', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const params = request.params as Record<string, string>;

    ...
    schema: {
      ...
      querystring: {
        ...
        properties: {
          ...
          source: { type: 'string', enum: ['contact', 'demo', 'all'], default: 'all' }
        }
      }
    }
  }, async (request, reply) => {
    try {
      const { limit = 100, offset = 0, source = 'all' } = request.query;

      let query = db.select().from(contactSubmissions);

      // Filter by source if specified
      if (source !== 'all') {
        query = query.where(eq(contactSubmissions.source, source));
      }
      ...
      if (source !== 'all') {
        countQuery = countQuery.where(eq(contactSubmissions.source, source));
      }

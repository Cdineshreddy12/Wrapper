  ...
  fastify.get('/payment/:identifier', {
    ...
    try {
      ...
      if (identifier.startsWith('mock_subscription_session_') || identifier.startsWith('mock_')) {
        const planId = request.query?.planId || 'starter';
        const billingCycle = request.query?.billingCycle || 'yearly';
      ...
      try {
        ...
        const planId = payment.planId || payment.metadata?.planId || payment.metadata?.packageId;
        const planAccess = planId ? PLAN_ACCESS_MATRIX[planId] : null;
        ...
        const billingCycle = payment.metadata?.billingCycle || payment.billingCycle || 'yearly';
        return {
          success: true,
          data: {
            sessionId: payment.stripePaymentIntentId || payment.metadata?.stripeCheckoutSessionId,

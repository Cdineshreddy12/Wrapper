  ...
  fastify.get('/profile-status', {
    ...
    try {
      ...
      const hasRequiredFields = !!(
        ...
        (tenant.industry || tenant.businessType) && // Use businessType as fallback
            ...
            await OnboardingTrackingService.trackOnboardingPhase(
              ...
              'completed',
              {
                ...
                eventData: {
                  ...
                  hasCompanyInfo: !!(legalCompanyName && (industry || tenant.businessType)),
  ...
  fastify.post('/purchase-credits', {
    ...
    try {
      ...
      const creditPackageLimits = {
        basic: {
          tools: ['crm'],
          users: Math.min(Math.floor(credits / 50), 25),
          roles: Math.min(Math.floor(credits / 100), 10),
          projects: Math.min(Math.floor(credits / 20), 100)
        },
        standard: {
          tools: ['crm', 'hr'],
          users: Math.min(Math.floor(credits / 40), 50),
          roles: Math.min(Math.floor(credits / 80), 15),
          projects: Math.min(Math.floor(credits / 15), 200)
        },
        premium: {
          tools: ['crm', 'hr', 'affiliate'],
          users: Math.min(Math.floor(credits / 30), 100),
          roles: Math.min(Math.floor(credits / 60), 20),
          projects: Math.min(Math.floor(credits / 10), 500)
        },
        enterprise: {
          tools: ['crm', 'hr', 'affiliate', 'accounting', 'inventory'],
          users: Math.min(Math.floor(credits / 20), 500),
          roles: Math.min(Math.floor(credits / 40), 50),
          projects: Math.min(Math.floor(credits / 5), 1000)
        }
      };

...
export class SubscriptionService {
  ...
  static async getCurrentSubscription(tenantId) {
    try {
      ...
      try {
        ...
        console.warn('⚠️ Error getting credit balance for subscription:', balanceError?.message);
        creditBalance = await CreditService.getCurrentBalance(tenantId);
      }

      const finalAvailableCredits = creditBalance?.availableCredits ?? 0;
      const finalTotalCredits = creditBalance?.availableCredits ?? creditBalance?.totalCredits ?? 0;
      ...
      return {
        ...
        monthlyPrice: actualSubscription?.monthlyPrice != null ? parseFloat(actualSubscription.monthlyPrice) : 0,
        yearlyPrice: actualSubscription?.yearlyPrice != null ? parseFloat(actualSubscription.yearlyPrice) : 0,
      ...
      try {
        ...
        if (subscription && subscription.plan && subscription.plan !== 'free') {
          ...
          planUpgradeEntries.push({
            ...
            yearlyPrice: subscription.yearlyPrice ? parseFloat(subscription.yearlyPrice) : null,
            amount: subscription.yearlyPrice ? parseFloat(subscription.yearlyPrice) : 0,
            currency: 'USD',
            invoiceNumber: null
          });
        }
  ...
  static async createBillingPortalSession(tenantId, returnUrl) {
    if (!this.isStripeConfigured() || !stripe) {
  ...
  // NEW: Process immediate plan changes (downgrades/same-level only). Upgrades must use Billing Portal or Checkout.
  static async processImmediatePlanChange({ tenantId, currentSubscription, targetPlan, planId, billingCycle }) {
    try {
      ...
      if (currentSubscription.stripeSubscriptionId && this.isStripeConfigured()) {
        ...
        const portalUrl = await this.createBillingPortalSession(tenantId, returnUrl);
        return portalUrl;
      } else {
  ...
  static async updateAdminRestrictionsForPlan(existingRestrictions, newPlanId, planAccess) {
    try {
      ...
      // Remove restrictions that are no longer applicable (only if plan has limitations defined)
  ...
  static async handleCheckoutCompleted(session) {
    try {
      ...
      const billingCycle = (session.metadata?.billingCycle || 'yearly').toLowerCase();
      ...
      if (session.mode === 'subscription') {
        ...
        if (existingSubscription) {
          ...
          // Update existing subscription – only set columns that exist on subscriptions table
          const updateData = {
            plan: planId,
            status: 'active',
          ...
          // Create new subscription – only columns that exist on subscriptions table
        ...
        try {
          ...
          if (planCredits > 0) {
            console.log(`💰 Allocating ${planCredits} plan credits for ${plan.name} plan to tenant:`, tenantId);

            // Resolve tenant's default organization entity (same logic as getCurrentSubscription)
            const orgEntities = await db
              .select()
              .from(entities)
              .where(and(
            ...
            if (defaultEntity) {
              ...
              // Add credits with source 'subscription' so they're categorized as free credits
              await CreditService.addCreditsToEntity({
                ...
                source: 'subscription',
                sourceId: session.id || subscriptionRecord?.subscriptionId,
                description: `${plan.name} plan credits (${planCredits.toLocaleString()} annual credits)`,
                initiatedBy: '00000000-0000-0000-0000-000000000001' // System user ID
              });
            ...
            console.log(`ℹ️ Plan ${plan.name} has no included credits (${planCredits}), skipping credit allocation`);
          }
        } catch (creditError) {
          console.error('❌ Failed to allocate plan credits:', creditError.message);
          console.error('❌ Credit allocation error details:', creditError);
          // Don't throw - subscription is already saved, credits can be added manually if needed
        }
  ...
  static async _applyInvoicePaymentToSubscription(subscriptionRecord, stripeSubscriptionId, invoice) {
    const planId = invoice.lines?.data?.[0]?.price?.id
      ? await this.getPlanIdFromPriceId(invoice.lines.data[0].price.id)
      : null;
    const setPayload = {
    ...
    if (planId) {
      ...
      if (plan) {
        ...
        setPayload.yearlyPrice = String(plan.yearlyPrice ?? plan.price ?? 0);
      }
    }
    await db
      .update(subscriptions)
      .set(setPayload)
      .where(eq(subscriptions.subscriptionId, subscriptionRecord.subscriptionId));

    if (planId) {
      console.log(`🔄 Triggering role upgrade for plan: ${planId}`);
      await this.updateAdministratorRolesForPlan(subscriptionRecord.tenantId, planId);
      try {
        const onboardingOrgSetup = (await import('../../onboarding/services/onboarding-organization-setup.js')).default;
        await onboardingOrgSetup.updateOrganizationApplicationsForPlanChange(
      ...
      if (subscriptionId) {
        ...
        if (!subscription) {
          ...
          if (fallbackSubscription) {
            ...
            await this._applyInvoicePaymentToSubscription(fallbackSubscription, subscriptionId, invoice);
            return;
          }

          // Fallback: find tenant by Stripe customer ID (e.g. checkout ran but subscription row not yet linked)
          ...
          if (tenantByCustomer) {
            ...
            if (subByTenant) {
              ...
              await this._applyInvoicePaymentToSubscription(subByTenant, subscriptionId, invoice);
              return;
            }
          }

          // Fallback: resolve tenant from Stripe customer email (when tenant.stripeCustomerId was never set)
        ...
        // Resolve plan from invoice so we only apply plan changes after payment (e.g. after Billing Portal upgrade)
        const invoicePriceId = invoice.lines?.data?.[0]?.price?.id ?? null;
        const planId = invoicePriceId ? await this.getPlanIdFromPriceId(invoicePriceId) : null;
        const setPayload = {
          status: 'active',
          updatedAt: new Date()
        };
        if (planId) {
          ...
          if (plan) {
            setPayload.plan = planId;
            setPayload.subscribedTools = plan.applications ?? plan.subscribedTools ?? [];
            setPayload.usageLimits = plan.limits ?? plan.usageLimits ?? {};
            setPayload.yearlyPrice = String(plan.yearlyPrice ?? plan.price ?? 0);
          }
        }
        await db
        ...
        // Trigger role upgrade and org applications for the active plan
        if (planId) {
          console.log(`🔄 Triggering role upgrade for plan: ${planId}`);
          await this.updateAdministratorRolesForPlan(subscription.tenantId, planId);
          try {
            const onboardingOrgSetup = (await import('../../onboarding/services/onboarding-organization-setup.js')).default;
            const result = await onboardingOrgSetup.updateOrganizationApplicationsForPlanChange(
              subscription.tenantId,
              planId,
              { skipIfRecentlyUpdated: true }
            );
            console.log('✅ Organization applications updated for new plan:', result.applicationsAssigned ?? 0, 'assigned');
          } catch (orgAppError) {
            console.error('❌ Failed to update organization applications:', orgAppError.message);
          }
        }
  ...
  static async handleSubscriptionUpdated(subscription) {
    try {
      ...
      const planId = priceId ? await this.getPlanIdFromPriceId(priceId) : null;
      ...
      if (existing?.tenantId && planId) {
        ...
        await this.updateAdministratorRolesForPlan(existing.tenantId, planId);
        try {
          const onboardingOrgSetup = (await import('../../onboarding/services/onboarding-organization-setup.js')).default;
  ...
  /**
   * Get payment details from a Stripe checkout session ID (real implementation like top-ups).
   * Used when the success page calls GET /payment/cs_xxx before the webhook has created the payment record.
   */
  static async getPaymentDetailsByCheckoutSessionId(sessionId, tenantId) {
    if (!stripe || !this.isStripeConfigured()) return null;
    try {
      ...
      console.error('getPaymentDetailsByCheckoutSessionId:', err?.message);
      return null;
    }
  }
    ...
    try {
      ...
      if (newPlan === 'trial' && currentSubscription.stripeSubscriptionId) {
        ...
        if (targetPlan && targetPlan.stripePriceId) {
          const stripeSubscription = await stripe.subscriptions.retrieve(currentSubscription.stripeSubscriptionId, { expand: ['items.data'] });
      ...
      await db
        ...
        .set({
          ...
          subscribedTools: targetPlan?.applications ?? ['crm'],
          usageLimits: targetPlan?.limits ?? { users: 2, apiCalls: 1000, storage: 1000000000, projects: 3 },
          yearlyPrice: targetPlan != null ? String(targetPlan.yearlyPrice ?? targetPlan.price ?? 0) : '0',

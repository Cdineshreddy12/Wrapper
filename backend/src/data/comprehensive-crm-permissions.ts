...
function getAllPermissionsFlat(): string[] {
...
function getPermissionsByAction(action: string): string[] {
  return getAllPermissionsFlat().filter((p: string) => p.includes(`.${action}`));
}

// Get permissions by pattern
function getPermissionsByPattern(pattern: string): string[] {
  return getAllPermissionsFlat().filter((p: string) => p.includes(pattern));
}

// Get permissions for specific module
function getModulePermissions(module: keyof typeof CRM_PERMISSION_MATRIX): string[] {
...
export class CRMPermissionUtils {
  ...
  static getModulePermissions(module: keyof typeof CRM_PERMISSION_MATRIX): string[] {
    return Object.keys(CRM_PERMISSION_MATRIX[module] || {});
  }

  // Get permissions for a role template
  static getRolePermissions(roleTemplate: { permissions?: Record<string, unknown> }): string[] {
    return this.getAllPermissions().filter((permission: string) => 
  ...
  static getPermissionDescription(permission: string): string {
    ...
    for (const modulePerms of Object.values(CRM_PERMISSION_MATRIX)) {
      if (permission in modulePerms) {
        return (modulePerms as Record<string, string>)[permission];
      }
    }
    
    // Check in special permissions
    if (permission in CRM_SPECIAL_PERMISSIONS) {
      return CRM_SPECIAL_PERMISSIONS[permission as keyof typeof CRM_SPECIAL_PERMISSIONS];
    }
  ...
  static generatePermissionsObject(permissionsList: string[]): Record<string, { granted: boolean; description: string; grantedAt: string }> {
    const permissionsObj: Record<string, { granted: boolean; description: string; grantedAt: string }> = {};

...
class AmazonMQPublisher {
   ...
   */
  async publishRoleEventToSuite(eventType, tenantId, roleId, roleData, publishedBy = 'system') {
    const results = [];
    for (const targetApp of this.businessSuiteApps) {
      try {
        ...
        results.push({ targetApp, success: false, error: error.message });
      }
    }
    return results;
  }
   ...
   */
  async publishUserEventToSuite(eventType, tenantId, userId, userData, publishedBy = 'system') {
    const results = [];
    for (const targetApp of this.businessSuiteApps) {
      try {
   ...
   */
  async publishOrgEventToSuite(eventType, tenantId, orgId, orgData, publishedBy = 'system') {
    const results = [];
    for (const targetApp of this.businessSuiteApps) {
      try {
   ...
   */
  async publishOrgAssignmentEventToSuite(eventType, tenantId, assignmentData, publishedBy = 'system') {
    const results = [];
    for (const targetApp of this.businessSuiteApps) {
      try {

...
class DistributedSSOCache {
   ...
   */
  _getAuthKey(kindeUserId, orgCode) {
   ...
   */
  _getPermissionsKey(userId, tenantId, app) {
   ...
   */
  async getUserAuth(kindeUserId, orgCode) {
   ...
   * Cache user auth
   */
  async cacheUserAuth(kindeUserId, orgCode, authData, ttl = 3600) {
    const key = this._getAuthKey(kindeUserId, orgCode);
   ...
   */
  async getUserPermissions(userId, tenantId, app) {
   ...
   */
  async cacheUserPermissions(userId, tenantId, app, permissions, ttl = 3600) {
    const key = this._getPermissionsKey(userId, tenantId, app);
    const expiresAt = Date.now() + (ttl * 1000);
   ...
   * Cache user roles
   */
  async cacheUserRoles(userId, tenantId, roles, ttl = 3600) {
   ...
   */
  async invalidateUserCache(identifier, tenantId) {

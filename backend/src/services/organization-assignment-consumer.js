...
// Redis removed - using AWS MQ consumer instead
...
class OrganizationAssignmentConsumer {
  ...
  async connect() {
    // Redis removed - using AWS MQ consumer instead
    console.log('⚠️ Redis consumer disabled - using AWS MQ consumer instead');
    throw new Error('Redis removed - please use AWS MQ consumer (amazon-mq-consumer.js)');
  }

  async setupConsumerGroup() {
    // Redis removed - consumer groups handled by AWS MQ
    console.log('⚠️ Redis consumer group setup disabled - AWS MQ handles this');
  }

...
export class OrganizationService {
  ...
  async createSubOrganization(data, createdBy) {
    const { name, description, gstin, parentOrganizationId, organizationType, tenantId, responsiblePersonId } = data;
    ...
    if (!finalResponsiblePersonId) {
      ...
      try {
        const [tenantAdmin] = await db
          ...
          .from(tenantUsers)
          .where(and(
            eq(tenantUsers.tenantId, tenantIdToUse),
        ...
        if (tenantAdmin) {
          ...
          console.log('👑 Using tenant admin as default responsible person:', tenantAdmin.userId);
        } else {
          console.log('⚠️ No tenant admin found, using creator as responsible person');
          finalResponsiblePersonId = createdBy;
        }
      } catch (error) {
        console.error('❌ Error finding tenant admin, using creator:', error);
        finalResponsiblePersonId = createdBy;
      }
    }

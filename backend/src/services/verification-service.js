    ...
    try {
      ...
      try {
        ...
        if (error.response) {
          ...
          const isAuthOrConfigError = /Authentication failed|401|Subscription (has )?expired|Invalid API credentials/i.test(error.message || '');
          return {
            verified: false,
            error: msg,
            details: null,
      ...
      // Auth/API config failures (401, subscription expired, etc.) should not block onboarding
      const isAuthOrConfigError = /Authentication failed|401|Subscription (has )?expired|Invalid API credentials/i.test(msg) ||

...
class DNSManagementService {
  ...
  constructor() {
    ...
    // Initialize AWS SDK v3 if credentials are available
    this.initializeAWS();
  }
  ...
  async getDNSRecord(domain, type) {
    ...
    try {
      const result = await this.route53.send(new ListResourceRecordSetsCommand(params));

      const record = (result.ResourceRecordSets || []).find(
        r => r.Name === (domain.endsWith('.') ? domain : `${domain}.`) && r.Type === type
      );
  ...
  async getChangeStatus(changeId) {
    try {
      const result = await this.route53.send(new GetChangeCommand({ Id: changeId }));
  ...
  async healthCheck() {
    try {
      ...
      const zones = await this.route53.send(new ListHostedZonesCommand({ MaxItems: 1 }));

    ...
    try {
      ...
      for (const allocation of allocations) {
        try {
          ...
          // Publish credit allocation event to AWS MQ
          // This mimics what would have been published when the allocation was created
          const publishResult = await amazonMQPublisher.publishCreditAllocation(
            'crm',
            tenantId,
    ...
    try {
      ...
      for (const org of orgs) {
        try {
          ...
          const publishResult = await amazonMQPublisher.publishOrgEvent(
            'crm',
            'org_created',
            tenantId,
            org.entityId,
    ...
    try {
      ...
      for (const assignment of assignments) {
        try {
          ...
          // Publish organization membership event to AWS MQ
          const publishResult = await amazonMQPublisher.publishOrgEvent(
            'crm',

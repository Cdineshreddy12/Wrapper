...
export default async function enhancedInternalRoutes(fastify, options) {
  ...
  fastify.get('/api/metrics/business-dashboard', async (request, reply) => {
    try {
      ...
      const { UsageCache } = await import('../utils/usage-cache.js');
      ...
      // Redis removed - using in-memory cache
      // Cache is now handled by UsageCache (in-memory)
      console.log('💾 Generating fresh business dashboard metrics');
      
      // Get important feature metrics
      const { importantFeatureTracker } = await import('../middleware/important-feature-tracker.js');
      const metrics = importantFeatureTracker.getImportantFeatureMetrics();
      
      // Get real-time usage data from in-memory cache
      ...
      // Get performance trends from in-memory cache
      const performanceTrends = [];
      
      // Generate time-based performance data from actual usage
      const generatePerformanceTrends = async () => {
        ...
        for (let i = 5; i >= 0; i--) {
          ...
          // Get cached performance metrics for this interval (from UsageCache)
          const intervalMetrics = await UsageCache.storeIntervalMetrics(tenantId, timeSlot.getTime(), {});
      ...
      const businessMetrics = {
        ...
        applicationPerformance: await (async () => {
          ...
          for (const [app, data] of Object.entries(metrics.applicationUsage)) {
            ...
            // Redis removed - using UsageCache for app metrics
            const appMetrics = await UsageCache.storeAppMetrics(tenantId, app, {}) || {};
      ...
      // Redis removed - caching disabled (metrics generated on-demand)
      // Track this API call

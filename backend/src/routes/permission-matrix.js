  ...
  fastify.get('/user-context', {
    ...
    try {
      ...
      const flattenedPermissions = typeof rawPermissions === 'object' && !Array.isArray(rawPermissions)
        ? PermissionMatrixService.flattenNestedPermissions(rawPermissions)
        : Array.isArray(rawPermissions) ? rawPermissions : [];
      
      console.log(`🔧 Permission flattening result:`, {
        originalPermissionsCount: typeof rawPermissions === 'object' && !Array.isArray(rawPermissions) ? Object.keys(rawPermissions).length : (rawPermissions?.length ?? 0),

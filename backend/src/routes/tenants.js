...
export default async function tenantRoutes(fastify, options) {
  ...
  fastify.get('/current/timeline', async (request, reply) => {
    if (!request.userContext?.isAuthenticated) {
      return reply.code(401).send({ error: 'Unauthorized' });
    }
    ...
    try {
      ...
      const { limit = 300, includeActivity = 'true' } = request.query;
      const shouldIncludeActivity = includeActivity === 'true' || includeActivity === true;
      ...
      if (subscription && subscription.createdAt) {
        ...
        const planDisplayName = planNameMap[subscription.plan] || subscription.plan.charAt(0).toUpperCase() + subscription.plan.slice(1);
      ...
      for (const purchase of creditPurchasesList) {
        ...
        if (purchaseDate && purchase.status === 'completed') {
          const credits = parseFloat(purchase.creditAmount || 0);
    ...
    schema: {
      body: {
        ...
        properties: {
          ...
          billingCountry: { type: 'string', maxLength: 100 },
      ...
      if (newRoleIds.length > 0 && newAssignments.length > 0) {
        try {
          ...
          for (const assignment of newAssignments.filter(a => newRoleIds.includes(a.roleId))) {
            try {
              const { amazonMQPublisher } = await import('../utils/amazon-mq-publisher.js');
              await amazonMQPublisher.publishRoleEventToSuite('role_assigned', tenantId, assignment.roleId, {
                assignmentId: assignment.id,
                userId: userId,
                roleId: assignment.roleId,

...
export default async function onboardingRoutes(fastify, options) {
  ...
  fastify.post('/create-organization', {
    ...
    try {
      ...
      const addResult = await kindeService.addUserToOrganization(kindeUserId, orgCode, { exclusive: true });
      if (addResult?.success) {
        console.log(`✅ [${requestId}] User successfully added to organization: ${orgCode}`);
      } else {
        console.warn(`⚠️ [${requestId}] addUserToOrganization returned unsuccessful response`, addResult);
      }

      // Post-assignment cleanup: ensure user is ONLY in the new organization
      try {
        const postAssignOrgs = await kindeService.getUserOrganizations(kindeUserId);
        const otherOrgs = (postAssignOrgs.organizations || []).filter(o => o.code !== orgCode);
        if (otherOrgs.length > 0) {
          console.log(`🧹 [${requestId}] Removing user from ${otherOrgs.length} other organizations after assignment...`);
          for (const org of otherOrgs) {
            try {
              await kindeService.removeUserFromOrganization(kindeUserId, org.code);
              console.log(`✅ [${requestId}] Removed user from organization: ${org.code}`);
            } catch (removeError) {
              console.warn(`⚠️ [${requestId}] Failed to remove user from org ${org.code} during post-cleanup:`, removeError.message);
            }
          }
        } else {
          console.log(`✅ [${requestId}] User is only in the new organization as expected.`);
        }
      } catch (verifyError) {
        console.warn(`⚠️ [${requestId}] Could not verify post-assignment organizations:`, verifyError.message);
      }
